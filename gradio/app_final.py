...
def analisar_texto(texto: str):
    ...
    frequencia_html = "<br>".join(
        [f"{palavra}: {contagem}" for contagem, palavra in frequencia.items()]
    )
...
def criar_relatorio(temperatura, escala, condicao, texto):
    ...
    relatorio = (
        ...
        f"Frequência de Caracteres: {frequencia}"
    )
    return relatorio
...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Text(label="Descrição do Dia:", lines=4, placeholder="Descreva o dia"),
    ],
